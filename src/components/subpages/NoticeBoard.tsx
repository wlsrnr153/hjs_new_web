...
const NoticeBoard = () => {
  ...
  const handleCategoryChange = (category: string) => {
    ...
    if (category === '전체') {
      ...
      // 상태에 따라 필터링
      const filtered = allPosts.filter(post => 
        category === '중요' ? post.status === '중요' : post.status !== '중요'
      );
      setPosts(filtered);
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {/* 페이지 제목 */}
      <div className="mb-10">
        <h1 className="text-3xl font-bold text-gray-800 border-b pb-4">공지사항</h1>
      </div>
      ...
      <div className="mb-6 flex flex-col md:flex-row justify-between items-center">
        <div className="flex space-x-2 mb-4 md:mb-0">
          ...
          <button 
            className={`px-4 py-2 text-sm rounded-md ${selectedCategory === '중요' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
            onClick={() => handleCategoryChange('중요')}
          >
            중요
          </button>
          <button 
            className={`px-4 py-2 text-sm rounded-md ${selectedCategory === '일반' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
            onClick={() => handleCategoryChange('일반')}
          >
            일반
          </button>
        </div>
      ...
      <div className="flex justify-end">
        <Link href="/notice/write" className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
          공지사항 작성
        </Link>
      </div>
    </div>
